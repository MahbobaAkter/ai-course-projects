...
print_results("vpn documents", results1)
...
print_results("High priority verified 2025 docs", results2)
...
results3a = collection.get(
    where_document={"$contains": "student"} 
)

print_results("Documents containing 'student'", results3a)


results3b = collection.get(
    where_document={
        "$and": [
            {"$contains": "student"},
...
print_results("Docs with 'student' but not 'password'", results3b)
...
print_results("Printing query with filters", results4, show_distances=True)